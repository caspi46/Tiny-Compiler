...
impl Block {
    ...
    fn push_head(&mut self, num_op: (i32, Operator)) {
        let new_inst = Rc::new(RefCell::new(Inst::new(num_op, None, self.head)));
        self.head = new_inst;
    }

    fn push_tail(&mut self, num_op: (i32, Operator)) {
        let new_inst = Rc::new(RefCell::new(Inst::new(nump_op, self.tail, None)));
        self.tail = new_inst;
    }

    fn get_head(&mut self) -> Link {
        head
    }

    fn get_tail(&mut self) -> Link {
        tail
    }
}
